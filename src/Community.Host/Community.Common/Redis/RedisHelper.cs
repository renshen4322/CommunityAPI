...
namespace Community.Common.Redis
{
    /// <summary>
    /// Redis操作
    /// </summary>
    public static class RedisHelper
    {
        ...
        private static string _customKey;
        private static readonly object RedisLock = new object();

        private static ConnectionMultiplexer RedisManager
        {
            get
            {
                if (_redis == null)
                {
                    lock (RedisLock)
                    {
                        ...
                        _redis = GetManager(GlobalAppSettings.RedisServerHost);
                        return _redis;
                    }
                }
        ...
        /// </summary>
        /// <param name="key"></param>
        /// <param name="start">起始区间</param>
        /// <param name="stop">结束区间</param>
        /// <param name="order">排序规则：0=降序|1=升序  默认为0</param>
        /// <returns></returns>
        public static List<T> SortedSetRangeByRank<T>(string key, long start, long stop, int order = 0)
        {
            key = AddSysCustomKey(key);
            return Do(redis =>
            {

                var values = redis.SortedSetRangeByRank(key, start, stop, order == 0 ? Order.Descending : Order.Ascending);
        ...
        /// <param name="start">起始区间</param>
        /// <param name="stop">结束区间</param>
        /// <param name="exclude">排除边界值 0=包含start和stop|1=排除start|2=排除stop|3=排除start和stop</param>
        /// <param name="order">排序规则：0=降序|1=升序  默认为0</param>
        /// <returns></returns>
        public static List<T> SortedSetRangeByScore<T>(string key, long start, long stop, int exclude = 0, int order = 0)
        {
            key = AddSysCustomKey(key);
            return Do(redis =>
            {
                ...
                switch (exclude)
                {
                    case 0: ex = Exclude.None; break;
                    case 1: ex = Exclude.Start; break;
                    case 2: ex = Exclude.Stop; break;
                    case 3: ex = Exclude.Both; break;
                    default: ex = Exclude.None; break;
                }
                var values = redis.SortedSetRangeByScore(key, start, stop, ex, order == 0 ? Order.Descending : Order.Ascending);
        ...
        {
            ISubscriber sub = RedisManager.GetSubscriber();
        ...
        {
            var prefixKey = _customKey ?? GlobalAppSettings.SysCustomKey;
